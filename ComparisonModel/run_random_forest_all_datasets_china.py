...
class RandomForestBatchRunnerChina:
    def __init__(self):
        self.datasets = {
            6: 11,
            12: 17,
        ...
        self.config_file = Path('config/dataset_config_China.yaml')
        self.backup_file = Path('config/dataset_config_China.yaml.backup')
        self.python_exe = sys.executable
        
        self.results_summary = []
        
        self.log_file = f'rf_china_batch_run_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        
        print(f"\n{'='*100}")
        print(f"Random Forest Batch Runner (China Dataset)")
        print(f"{'='*100}")
    ...
    def modify_config(self, folder, channels):
        try:
            ...
            config['dataset']['train_file_list'] = 'Dataset/China/202101_train.txt'
            config['dataset']['val_file_list'] = 'Dataset/China/202101_val.txt'
            config['dataset']['test_file_list'] = 'Dataset/China/202101_test.txt'
            
            with open(self.config_file, 'w', encoding='utf-8') as f:
                yaml.dump(config, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
            
            self.log(f"Config updated: image_root=Dataset/China/HLS/{folder}, hls_channels={channels}")
            self.log(f"   File lists: 202101_train.txt, 202101_val.txt, 202101_test.txt")
            return True
    ...
    def run_single_dataset(self, folder, channels):
        ...
        train_cmd = (
            ...
            f"--train-file Dataset/China/202101_train.txt "
            f"--val-file Dataset/China/202101_val.txt "
            f"--test-file Dataset/China/202101_test.txt "
    ...
    def generate_excel_summary(self):
        ...
        try:
            ...
            output_file = Path('results/random_forest/all_experiments_summary_china.xlsx')
    ...
    def run_all(self):
        self.log(f"\n{'#'*100}")
        self.log(f"Starting batch run for Random Forest model (China Dataset)")
        self.log(f"Starting batch run for Random Forest model (China Dataset)")
        ...
        self.log(f"  - results/random_forest/all_experiments_summary_china.xlsx")
...
def main():
    try:
        runner = RandomForestBatchRunnerChina()
        runner.run_all()
        ...
        backup_file = Path('config/dataset_config_China.yaml.backup')
        config_file = Path('config/dataset_config_China.yaml')
