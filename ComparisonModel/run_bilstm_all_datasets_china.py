...
class BiLSTMBatchRunnerChina:
    def __init__(self):
        ...
        self.config_file = Path('config/dataset_config_china_bilstm.yaml')
        self.backup_file = Path('config/dataset_config_china_bilstm.yaml.backup')
        self.python_exe = sys.executable
        
        self.log_file = f'bilstm_china_batch_run_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        ...
        print(f"\n{'='*100}")
        print(f"BiLSTM Batch Runner (China Dataset, Two-Stage Training, Pixel-Level Classification)")
        print(f"{'='*100}")
    ...
    def generate_excel_summary(self):
        ...
        try:
            ...
            output_file = Path('results/bilstm/all_experiments_summary_china.xlsx')
    ...
    def run_all(self):
        self.log(f"\n{'#'*100}")
        self.log(f"Starting batch run for BiLSTM model (China Dataset, Two-Stage Training, Pixel-Level)")
        self.log(f"Starting batch run for BiLSTM model (China Dataset, Two-Stage Training, Pixel-Level)")
        ...
        self.log(f"  - results/bilstm/all_experiments_summary_china.xlsx")
...
def main():
    try:
        ...
        backup_file = Path('config/dataset_config_china_bilstm.yaml.backup')
        config_file = Path('config/dataset_config_china_bilstm.yaml')
