...
class TransformerBatchRunnerChina:
    def __init__(self):
        self.datasets = {
            6: 11,
            12: 17,
        ...
        self.original_config_file = Path('config/dataset_config_China.yaml')
        self.config_file = Path('config/dataset_config_china_transformer.yaml')
        self.backup_file = Path('config/dataset_config_china_transformer.yaml.backup')
        self.python_exe = sys.executable
        
        self.log_file = f'transformer_china_batch_run_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        
        self._init_config_file()
        
        self.epochs = 3
        
        self.historical_train = 'Dataset/China/train.txt'
        self.historical_val = 'Dataset/China/val.txt'
        self.historical_test = 'Dataset/China/test.txt'
        
        self.finetuned_train = 'Dataset/China/202101_train.txt'
        self.finetuned_val = 'Dataset/China/202101_val.txt'
        self.finetuned_test = 'Dataset/China/202101_test.txt'
        
        self.results_summary = []
        
        print(f"\n{'='*100}")
        print(f"Transformer Batch Runner (China Dataset, Two-Stage Training)")
        print(f"{'='*100}")
    ...
    def modify_config(self, folder, channels):
        try:
            ...
            config['dataset']['image_root'] = f'Dataset/China/HLS/{folder}'
            config['data_info']['hls_channels'] = channels
            
            with open(self.config_file, 'w', encoding='utf-8') as f:
                yaml.dump(config, f, allow_unicode=True, default_flow_style=False, sort_keys=False)
            
            self.log(f"Config updated: image_root=Dataset/China/HLS/{folder}, hls_channels={channels}")
            return True
    ...
    def extract_metrics(self, folder, stage='finetuned'):
        metrics_file = Path(f'results/transformer/{stage}_{folder}/metrics.txt')
    ...
    def run_single_dataset(self, folder, channels):
        ...
        self.log(f"\n{'='*100}")
        self.log(f"Stage 1/3: Train historical classifier (2019-2020 data)")
        self.log(f"Stage 1/3: Train historical classifier (2019-2020 data)")
        self.log(f"{'='*100}\n")
        ...
        self.log(f"\n{'='*100}")
        self.log(f"Stage 2/3: Fine-tune model (2021 data)")
        self.log(f"Stage 2/3: Fine-tune model (2021 data)")
        self.log(f"{'='*100}\n")
        
        if not self.modify_config(folder, channels):
            self.log(f"Stage 2 config check failed, skipping fine-tuning")
            return False
        
        historical_checkpoint = f"checkpoints/transformer/model_last_{historical_name}.pth"
        ...
        if not self.modify_config(folder, channels):
            self.log(f"Stage 3 config check failed, skipping test")
            return False
        
        finetuned_checkpoint = f"checkpoints/transformer/model_last_{finetuned_name}.pth"
    ...
    def generate_excel_summary(self):
        ...
        try:
            ...
            class_names = ['Rice', 'Corn', 'Soybeans', 'Other']
            ...
            output_file = Path('results/transformer_china/all_experiments_summary.xlsx')
    ...
    def run_all(self):
        self.log(f"\n{'#'*100}")
        self.log(f"Starting batch run for Transformer model (China Dataset, Two-Stage Training)")
        self.log(f"Starting batch run for Transformer model (China Dataset, Two-Stage Training)")
        ...
        for folder in self.datasets.keys():
            self.log(f"  - results/transformer/finetuned_{folder}/")
        self.log(f"  - results/transformer/all_experiments_summary_china.xlsx")
        
        self.log(f"\nCheckpoint locations:")
        self.log(f"Checkpoint locations:")
        for folder in self.datasets.keys():
            self.log(f"  - checkpoints/transformer/model_last_historical_{folder}.pth")
            self.log(f"  - checkpoints/transformer/model_last_finetuned_{folder}.pth")
...
def main():
    try:
        runner = TransformerBatchRunnerChina()
        runner.run_all()
        ...
        backup_file = Path('config/dataset_config_china_transformer.yaml.backup')
        config_file = Path('config/dataset_config_china_transformer.yaml')
