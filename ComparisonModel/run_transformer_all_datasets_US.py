...
class TransformerBatchRunner:
    def __init__(self):
        ...
        self.config_file = Path('config/dataset_config_transformer.yaml')
        self.backup_file = Path('config/dataset_config_transformer.yaml.backup')
        self.python_exe = sys.executable
        
        self.log_file = f'transformer_batch_run_log_{datetime.now().strftime("%Y%m%d_%H%M%S")}.txt'
        ...
        print(f"\n{'='*100}")
        print(f"Transformer Batch Runner (Two-Stage Training)")
        print(f"{'='*100}")
    ...
    def generate_excel_summary(self):
        ...
        try:
            ...
            output_file = Path('results/transformer/all_experiments_summary.xlsx')
    ...
    def run_all(self):
        self.log(f"\n{'#'*100}")
        self.log(f"Starting batch run for Transformer model (Two-Stage Training)")
        self.log(f"Starting batch run for Transformer model (Two-Stage Training)")
        ...
        self.log(f"  - results/transformer/all_experiments_summary.xlsx")
...
def main():
    try:
        ...
        backup_file = Path('config/dataset_config_transformer.yaml.backup')
        config_file = Path('config/dataset_config_transformer.yaml')
